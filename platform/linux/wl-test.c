...
wl_buffer_release(void *data, struct wl_buffer *wl_buffer)
{
  ...
  log("%s: begin\n", __func__);
  ctx->buffer_is_used_by_server = 0;
  log("buffer_is_used_by_server: %d\n", ctx->buffer_is_used_by_server);
...
int wayland_ctx_resize_buffer(void *vctx, int height, int width, int stride, int offset, uint32_t format)
{
  ...
  wl_buffer_add_listener(ctx->buffer, &wl_buffer_listener, ctx);
                                  ...
                                  uint32_t time)
{
  ...
  if (ctx->buffer_is_used_by_server) {
    ...
    for (int y = 0; y < 600; ++y) {
      for (int x = 0; x < 800; ++x) {
        if ((x + y / 8 * 8) % 16 < 8) {
	  ctx->pixels[y * 600 + x] = 0xFF000000 | (((time % 256) & 0xff) << 8);
        } else {
	ctx->pixels[y * 600 + x] = 0xFFFFFFFF;
        }
      }
    }
...
void wayland_ctx_free_shm_pool(void *vctx) {
  struct wayland_context *ctx = (struct wayland_context *)vctx;
  if (ctx->pool)
    wl_shm_pool_destroy(ctx->pool);
  if (ctx->pool_data && ctx->shm_pool_size != 0) {
...
void wayland_ctx_commit_buffer(void *vctx) {
  ...
  ctx->buffer_is_used_by_server = 1;
  wl_surface_commit(ctx->surface);
  log("%s, end\n", __func__);
}
...
void *wayland_ctx_make(void) {
  ...
  new->buffer = NULL;
  new->pixels = NULL;
  new->buffer_is_used_by_server = 0;
  new->height = 0;
  new->width = 0;
  return new;
}
...
int main(void) {
  ...
  int shm_pool_size = WIDTH * 4 * HEIGHT * 2;
  log("%s, begin\n", __func__);
  ...
  log("%s: %d\n", __func__, __LINE__);
  ret = wayland_ctx_create_surface(ctx, "wl_test");
  if (ret) {
  ...
  wayland_ctx_resize_buffer(ctx, HEIGHT, WIDTH, WIDTH * 4, 0, WL_SHM_FORMAT_XRGB8888);
  wayland_ctx_commit_buffer(ctx);
